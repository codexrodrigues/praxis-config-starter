...
- `contextHints.domainCatalog` e um subcontrato versionado (`schemaVersion=praxis.ai.context-hints.domain-catalog/v0.1`) para solicitar contexto semantico/governanca no proximo turno. Os campos canonicos sao `serviceKey`, `type`, `query`, `contextKey`, `nodeType` e `limit`.
