...
- `contextHints.domainCatalog` e um subcontrato versionado (`schemaVersion=praxis.ai.context-hints.domain-catalog/v0.1`) para solicitar contexto semantico/governanca no proximo turno. Os campos canonicos sao `serviceKey`, `resourceKey`, `releaseId`, `releaseKey`, `type`, `itemTypes`, `intent`, `query`, `contextKey`, `nodeType`, `recommendedOperation` e `limit`.
