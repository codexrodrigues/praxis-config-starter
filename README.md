...
When a caller sends `authoringContract.responseModes`, the backend also promotes those modes to intent classification. A consultative mode with `preferredResponse: "info"` tells the LLM to answer capability, schema, endpoint, styling and how-to questions as information instead of turning missing edit parameters into clarification. Edit/materialization turns remain governed by `componentEditPlan`; this routing must be semantic LLM classification, not keyword checks in UI runtimes.
