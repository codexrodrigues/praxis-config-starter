...
- Em `/api/praxis/config/domain-knowledge/change-sets/{changeSetId}/apply`, a aplicação de Domain Knowledge é uma fronteira separada da aprovação: o change set precisa estar `approved`, manter `validation_result=valid` e passar por escopo tenant/environment antes de materializar alterações. O primeiro corte canônico aplica apenas `add_evidence` contra conceito existente no mesmo escopo, registra evidência governada e então move o change set para `applied`; demais operações devem continuar bloqueadas até terem semântica transacional própria.
- `GET /api/praxis/config/domain-knowledge/change-sets/{changeSetId}/timeline` expõe uma timeline segura derivada do change set, validação, revisão e aplicação. A resposta deve permanecer `visibility=safe`, sem retornar patch bruto, payload de evidência, `sourcePointer`, `sourceUri`, `patchHash`, prompt ou histórico de chat.
