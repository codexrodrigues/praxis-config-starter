...
Status: deferred until a coordinated Maven + quickstart rollout is explicitly requested.
...
The timeline is not a new source of truth. It is a derived observability projection over the canonical domain-rule definition and materializations.
...
Do not publish Maven Central only because the endpoint exists on `main`.
...
- a published quickstart/runtime proof is required for external documentation or demos.
...
4. Against a quickstart packaged with the unreleased starter, run the local timeline lane:
