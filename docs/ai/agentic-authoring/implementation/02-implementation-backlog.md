...
Slice 5 concluido em 2026-05-01 para o baseline vector-disabled: o quickstart
foi empacotado contra o starter local, executado em `http://localhost:8099`
contra Neon com publicacao vetorial desabilitada e validado por HTTP real. O
smoke com `REQUIRE_PROJECT_KNOWLEDGE_RETRIEVAL=true` e
`REQUIRE_EVIDENCE_REVERT=true` confirmou Project Knowledge presente apos
`add_evidence` e ausente apos `revert_evidence`. O smoke com
`REQUIRE_EVIDENCE_SUPERSESSION=true` confirmou criacao de evidencia substituta e
timeline de supersession. Prova vector-enabled permanece bloqueada ate existir
implementacao real de indice derivado que preserve re-check canonico.
